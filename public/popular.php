...
<head>
    ...
    <title>Dine | Popular Recipes </title>
</head>
...
<body>
    ...
    <main>
        ...
        <section class="top">
            ...
            <p>Find the popular recipes created from the users at Dine. From vegetarian lunch ideas, to tasty bakery recipes and more. </p>
        </section>

        <section class="most-liked">
            <?php 
                $sql = "SELECT idrecipe, name, time, difficulty, picture, imageType, COUNT(*) as total from likes, recipe where idrecipe = idr group by idr HAVING total >= 2 ORDER BY total DESC;";
                $stmt = $db->prepare($sql); 
                $stmt->execute(); 
                if ($stmt ->rowCount() > 0) {
